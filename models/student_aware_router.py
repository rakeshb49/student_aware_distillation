...
class StudentCapacityEstimator(nn.Module):
    ...
    def __init__(self, student_dim: int, teacher_dim: int, num_experts: int = 8):
        ...
        self.gap_analyzer = nn.Sequential(
            nn.Linear(student_dim + teacher_dim, teacher_dim),
    ...
    def forward(self, student_hidden: torch.Tensor, 
        ...
        if teacher_hidden is not None:
            # Project teacher hidden to match dimensions if needed
            if teacher_hidden.size(-1) != self.teacher_dim:
                teacher_proj = F.adaptive_avg_pool1d(
                    teacher_hidden.transpose(1, 2), self.teacher_dim
                ).transpose(1, 2)
            else:
                teacher_proj = teacher_hidden
                
            # Concatenate and analyze gaps
            combined = torch.cat([
                student_hidden,
                F.adaptive_avg_pool1d(teacher_proj.transpose(1, 2), 
                                     self.student_dim).transpose(1, 2)
            ], dim=-1)
...
class AdaptiveExpertRouter(nn.Module):
    ...
    def compute_routing_weights(self, 
        ...
        # Compute expert importance from teacher
        expert_importance = self.expert_scorer(teacher_hidden)
    ...
    def forward(self, student_hidden: torch.Tensor,
        ...
        expert_stack = torch.stack(teacher_expert_outputs, dim=2)  # [B, L, E, D]
        
        # Use first expert output as representative for routing
        representative_teacher = teacher_expert_outputs[0]
